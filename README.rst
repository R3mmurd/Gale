...
- `gale.event <https://github.com/R3mmurd/Gale/blob/main/docs/examples/event.rst>`_: decoupled publish/subscribe — Signal, EventEmitter, and a global EventBus.
...
- `gale.memoize <https://github.com/R3mmurd/Gale/blob/main/docs/examples/memoize.rst>`_: Memoized and Memo, a game-time-driven cache (forever, per-frame, or TTL) auto-updated by Game alongside Timer.
...
- `gale.particle_system <https://github.com/R3mmurd/Gale/blob/main/docs/examples/particle_system.rst>`_: geometric shapes, textures, or a mix of both per burst.
...
- `gale.timer <https://github.com/R3mmurd/Gale/blob/main/docs/examples/timer.rst>`_: groups, per-item pause/progress, and During for a raw per-frame callback.
...
- ``gale.event``: Contains a decoupled publish/subscribe toolkit — ``Signal`` (a single event channel: ``connect``/``disconnect``/``emit``, priority-ordered, with a ``once`` option), ``EventEmitter`` (a mixin/composable object owning any number of ad hoc, string-named Signals), and ``EventBus`` (a ready-to-use global ``EventEmitter``, classmethods over a single instance, the same shape ``InputHandler`` uses). Every ``emit`` isolates each listener's exceptions (logged through ``gale.log``, never propagated), so one broken subscriber never breaks another. (`example <https://github.com/R3mmurd/Gale/blob/main/docs/examples/event.rst>`__)
...
- ``gale.memoize``: Contains ``Memoized`` (wraps a function, caching its return value per distinct argument combination) and ``Memo`` (a process-wide registry of every ``Memoized`` created through ``Memo.memoize``, aged by ``Memo.update(dt)`` the same way ``Timer`` ages ``Every``/``After``/``Tween`` — ``gale.game.Game`` calls both automatically). ``ttl`` is measured in seconds of game time, not the wall clock: ``None`` caches forever, ``0`` caches for the rest of the current frame, and a positive number expires the entry after that much accumulated ``dt`` — so, unlike ``functools.lru_cache``, an entry decorating an instance method doesn't pin that instance in memory forever. (`example <https://github.com/R3mmurd/Gale/blob/main/docs/examples/memoize.rst>`__)
...
- ``gale.particle_system``: Contains ``Particle``/``ParticleSystem`` for explosions, sparks, trails, and similar burst effects. Each particle renders as a geometric shape (``SHAPE_CIRCLE`` by default, plus ``SHAPE_SQUARE``/``SHAPE_TRIANGLE``/``SHAPE_DIAMOND``/``SHAPE_STAR``/``SHAPE_LINE`` — see ``PARTICLE_SHAPES``) or a texture (any ``pygame.Surface``, tinted by the particle's own color); ``set_shapes``/``set_textures`` let one burst mix both, each particle drawing its own appearance from the combined pool the same way it already draws its own color from ``set_colors``. ``set_size``/``set_angular_velocity`` add a size range and a spin range on top. (`example <https://github.com/R3mmurd/Gale/blob/main/docs/examples/particle_system.rst>`__)
...
- ``gale.timer``: Contains ``Every``/``After``/``Tween``/``During`` (a repeating callback, a one-shot delayed callback, an eased attribute interpolation, and a raw per-frame callback fed ``dt``/``progress``, respectively) and ``Timer``, the process-wide scheduler managing all of them. Any item can be tagged with a ``group`` (a string, or any other hashable, even the owning object itself) so ``Timer.pause``/``resume``/``clear`` can target just that subset; an item also has its own ``.pause()``/``.resume()``/``.progress``, and ``ignore_global_pause`` exempts it from a groupless ``Timer.pause()`` (for a UI's own timers, say, which usually shouldn't freeze just because gameplay did). (`example <https://github.com/R3mmurd/Gale/blob/main/docs/examples/timer.rst>`__)
