...
- ``gale.game``: Contains a base class ``Game`` to be inherited to ease your game building — a variable-rate ``update``/``render`` pair driven by real elapsed time, plus a ``fixed_update`` that steps at a constant rate regardless of frame rate, for anything that needs to be frame-rate-independent (e.g. driving a ``gale.physics.World``).
...
.. |GithubCommits| image:: https://img.shields.io/github/commits-since/R3mmurd/Gale/v1.10.0.svg
   :target: https://github.com/R3mmurd/Gale/compare/v1.10.0...main
