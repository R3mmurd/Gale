...
Kinematic target that a Seek/Arrive steering behavior can be built
against once and reused for the whole path, since only the target's
position is mutated as the path is followed. Adapted from
examples/nightwatch/src/path_follower.py for the AI racer in
src/ai_car.py.
"""
...
class PathFollower:
    ...
    def update(self, position: pygame.Vector2) -> None:
        """
        Advance to the next waypoint if position is already close
        enough to the current one.
