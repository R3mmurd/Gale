...
class LobbyState(BaseState):
    """
    Waits for the other player to show up: as host, listens for one
    connection and advertises itself on the LAN; as a joining player,
    lets you type a "host:port" or scan the LAN for one and connect.
    """
    ...
    def _enter_as_host(self) -> None:
        ...
        self.server.enable_lan_discovery(
            "Rally Table", discovery_port=settings.DEFAULT_DISCOVERY_PORT
        )
        self.server.on_connect(self._on_opponent_connected)

        self.status_label = Label(
            settings.VIRTUAL_WIDTH / 2,
            settings.VIRTUAL_HEIGHT / 2,
            f"Hosting on port {self.server.port}. Waiting for an opponent...",
    ...
    def _build_manager(self, root: Container) -> None:
        self.ui = UIManager(
            root,
    ...
    def _on_opponent_connected(self, peer) -> None:
